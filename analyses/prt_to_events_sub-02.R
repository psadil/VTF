...
files <- list.files(pattern = glue::glue( "AA.*[[:digit:]].prt"), recursive = TRUE, full.names = TRUE )
files <- c(files, list.files(pattern = glue::glue( "TR2000_PRT"), recursive = TRUE, full.names = TRUE ))
...
d <- file.info(files, extra_cols = FALSE) %>%
  ...
  mutate(task = case_when(str_detect(filename, "loc") ~ "loc",
                          str_detect(filename, "TR2000_PRT") ~ "ret",
                          str_detect(filename, "[[:punct:]]r[[:digit:]].prt") ~ "con"),
         run = case_when(task == "ret" ~ "1",
                         TRUE ~ str_extract(filename, "[[:digit:]].prt")),
...
for(r in unique(con$run)){
  con %>%
    filter(run == r) %>%
    readr::write_tsv(., file.path(glue::glue("sub-02_task-con_run-{sprintf('%02d', r)}_events.tsv")))
}
...
ret <- d %>%
  ...
  mutate(onset = map2_dbl(onset, repetition, ~.x[.y])/1000,
         offset = map2_dbl(offset, repetition, ~.x[.y])/1000,
         duration = offset - onset,
         subject = 2,
         trial_type = plyr::mapvalues(trial_id, c("HM","VM"), c("horizontal", "vertical"))) %>%
...
for(r in unique(ret$run)){
  ret %>%
    filter(run == r) %>%
    readr::write_tsv(., file.path(glue::glue("sub-02_task-ret_run-{sprintf('%02d', r)}_events.tsv")))
}
...
for(r in unique(loc$run)){
  loc %>%
    filter(run == r) %>%
    readr::write_tsv(., file.path(glue::glue("sub-02_task-loc_run-{sprintf('%02d', r)}_events.tsv")))
}
