...
params:
  ...
  run: !r c(1, 2)
  flip_hz: 10
  extra_flip: 0 # number of extra flips within a trial (e.g., at start of trial, end of trial)
  n_phase: 16
  scan_dur_sec: 420
---
...
d_main <- finfo %>%
  ...
  mutate(data = map(data, ~.x %>% 
                      arrange(trial) %>% 
                      mutate(iti_expected = case_when(trial_type == "grating" | trial > 1 ~ dplyr::lead(onset_expected, default = params$scan_dur_sec) - duration_expected - onset_expected,
                                                      trial_type == "dim" & trial == 1 ~ dplyr::lead(onset_expected, default = params$scan_dur_sec) - duration_expected )))) %>%
  unnest() %>%
  mutate(n_flip_in_trial = (duration_expected + iti_expected) * params$flip_hz
         , trial_start = NaN # in system time
         , trial_end = NaN # in system time
...
d_tInfo <- d_main %>%
  select(-onset, -duration, -trial_start, -trial_end, -rt ) %>%
