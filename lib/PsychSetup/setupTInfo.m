...
nFlips_total = stim.nFlipsPerTrial*expParams.nTrials + expParams.nTrials;

tInfo = table;
tInfo.flip_id = (1:nFlips_total)';

% trial includes final ITI flip at end of trial (that is the + 1)
tInfo.trial = repelem(1:expParams.nTrials, stim.nFlipsPerTrial + 1)';
tInfo.flipWithinTrial = repmat((1:(stim.nFlipsPerTrial + 1))', [expParams.nTrials, 1]);
...
tInfo.vbl_from0_expected = (0:stim.update_phase_sec:(stim.update_phase_sec*nFlips_total - stim.update_phase_sec))' + ...
    expParams.iti_dur_sec * repelem(0:expParams.nTrials-1, stim.nFlipsPerTrial+1)';
tInfo.missed = NaN(nFlips_total, 1);

% On every trial's flip, one of the two gratings changes phase
% NaN indicates neither is changing (happens on ITI flip)
whichGratingToFlipInTrial = [repmat(1:2, [1,stim.nFlipsPerTrial/2]), NaN]';
...
tInfo.phase_orientation_left(isnan(tInfo.whichGratingToFlip)) = NaN;
tInfo.phase_orientation_right(isnan(tInfo.whichGratingToFlip)) = NaN;
