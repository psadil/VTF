...
%{
I’m looking for some input on ideas for a way to draw some stimuli. 
The stimuli are sine gratings in an annulus shape, where contrast at the 
edges of the stimuli smoothly fade to gray. That’s all not too tough, but 
the challenge is that I’d also like the gratings to be presented at different 
orientations and for parts of the bounding rectangle of their annuli to be masked. 

Here's the solution I’ve gotten to so far. The basic idea was to hack 
together a smoothed annulus shader (based on the example of a smoothed sine 
grating shader at _______), draw them to an offscreen window, and then only 
draw part of the offscreen window to the main screen. That works (it’s how 
offscreen.png was generated), but I’m wondering whether there’s a way to 
draw the oriented gratings to the main window directly. 
...
try
    ...
    stim.fullWindowTex = Screen('MakeTexture', window.pointer,...
        ones(window.winRect(4), window.winRect(3)) .* window.gray, [], [], 2);
    % additive blending for these offscreen windows
    Screen('BlendFunction', stim.fullWindowTex, 'GL_ONE', 'GL_ONE');
    ...
    stim.srcRects = [0; 0; window.xCenter - stim.mid_strip_pix/2; window.winRect(4)];
    ...
    switch input.orient_method
        case 'offscreen'
            ...
            % draw
            Screen('DrawTextures', stim.fullWindowTex, stim.tex, [],...
            ...
            % window
            Screen('DrawTextures', window.pointer, stim.fullWindowTex, ...
                stim.srcRects, stim.srcRects);
            
        case 'onscreen'
            % ensure blending disabled in main window
            window.sourceFactor = 'GL_ONE';
            window.destinationFactor = 'GL_ONE';
            Screen('BlendFunction', window.pointer, window.sourceFactor, window.destinationFactor);
