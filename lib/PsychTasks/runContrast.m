        ...
        otherwise
            ...
            for trial = 1:expParams.nTrials
                ...
                switch input.experiment
                    case 'contrast'
                        angles = [tInfo.orientation_left(index_tInfo),...
                           tInfo.orientation_right(index_tInfo)];
                        contrasts = [tInfo.contrast_left(index_tInfo),...
                            tInfo.contrast_right(index_tInfo)];
                        phases = [tInfo{index_tInfo, contains(tInfo.Properties.VariableNames, 'phase_orientation_left_')},...
                            tInfo{index_tInfo, contains(tInfo.Properties.VariableNames, 'phase_orientation_right_')}];
                        spatial_frequency = stim.grating_freq_cpp;
                    case 'localizer'
                        angles = [tInfo.orientation_1(index_tInfo),...
                            tInfo.orientation_2(index_tInfo)];
                        contrasts = [tInfo.contrast(index_tInfo),...
                            tInfo.contrast(index_tInfo)];
                        phases = cat(3,tInfo{index_tInfo, contains(tInfo.Properties.VariableNames, 'phase_orientation_left_')},...
                            tInfo{index_tInfo, contains(tInfo.Properties.VariableNames, 'phase_orientation_right_')});
                        spatial_frequency = repmat(stim.grating_freq_cpp, [2,1])';
                end
                src_rects = CenterRectOnPointd([zeros(1,5); zeros(1,5); ...
                    stim.grating_aperture_pix*2; stim.grating_aperture_pix*2],...
                    window.xCenter, window.yCenter);
                for grating = 1:5
                    src_rects(:,grating) = CenterRectOnPointd(ScaleRect(src_rects(:,grating)', input.scale, input.scale),...
                        window.xCenter, window.yCenter);
                end
                ...
                [dimming_data.response_given(index_dimming), ...
                    ...
                    elicitContrastResp(...
                    stim.tex, spatial_frequency, src_rects, tInfo.vbl_expected_fromTrigger(index_tInfo),...
