function ga_contrast2(varargin)
...
addParameter(ip, 'min_onset_diff', 2.5, @isnumeric);
addParameter(ip, 'dim_dur', 0.4, @isnumeric);
addParameter(ip, 'dim_sep_max', 5, @isnumeric);
addParameter(ip, 'dim_sep_min', 3, @isnumeric);
addParameter(ip, 'duration', false, @islogical); % whether to try to optimize duration
addParameter(ip, 'epoch_length_max', 6, @isnumeric); % only takes effect when duration == false
...
n_stim_events = input.n_reps * n_stim_type + input.n_reps;
...
if input.duration
    ...
    % stim_type
    lb = zeros(1, n_stim_events);
    ub = ones(1, n_stim_events) * n_stim_type;
    n_vars = n_stim_events;
end
...
n_dim_events = floor(input.scan_time / (input.dim_dur + input.dim_sep_max));
...
population_parser = make_population_parser2(input.duration);
design_constraints_fcn = make_design_constraints(input.duration);
constraints = @(x)design_constraints_fcn(x, input.min_onset_diff, n_stim_events, epoch_length_max, population_parser);
    ...
    for run_to_save = input.runs
        ...
        while side_to_save <= 2
            ...
            for i = 1:input.PopulationSize
                trial_types(i,:) = Shuffle(repelem(0:n_stim_type, input.n_reps));
            end
            ...
            if input.duration                
                ...
                InitialPopulationMatrix = trial_types;
            end
            ...
            if exitflag > 0
                ...
                [stim_list, onsets, epoch_length] = ...
                    population_parser2(x, n_stim_events, epoch_length_max);
