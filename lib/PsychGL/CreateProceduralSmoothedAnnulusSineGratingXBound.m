function [gratingid, gratingrect] = CreateProceduralSmoothedAnnulusSineGratingXBound(windowPtr, width, height, backgroundColorOffset, radius, contrastPreMultiplicator, sigma, useAlpha, method, bound_x)
...
if all(isinf(radius))
    ...
    gratingShader = LoadGLSLProgramFromFiles('SineGratingSmoothedAnnulusShaderXBound', debuglevel);
end
...
if ~all(isinf(radius))
    ...
    glUniform1f(glGetUniformLocation(gratingShader, 'Radius_inner'), radius(1));
    glUniform1f(glGetUniformLocation(gratingShader, 'Radius_outer'), radius(2));
    % Apply sigma:
    glUniform1f(glGetUniformLocation(gratingShader, 'Sigma_inner'), sigma(1));
    glUniform1f(glGetUniformLocation(gratingShader, 'Sigma_outer'), sigma(2)); 
    % apply x bound
    glUniform1f(glGetUniformLocation(gratingShader, 'bound_x'), bound_x);
    % Apply useAlpha:
