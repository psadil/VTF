function rosieContrastPC

warning('off','MATLAB:dispatcher:InexactMatch');
% Screen('Preference', 'SkipSyncTests', 1);

%get subject info
prompt = {'Subject Name', 'Sub Number', 'Session Number', 'Run num', 'fMRI(1=Yes)','Contrast change [%,%]'};
...
if length(box)==length(defAns)      %simple check for enough input, otherwise bail out
   p.subName=char(box{1});p.subNum=str2num(box{2});p.sessNum=str2num(box{3});
   p.runNum=str2num(box{4});p.fMRI=str2num(box{5}); p.cChange=str2num(box{6})./100;
...
p.stimSizeDeg = 10;  % stim size in degrees (radius)
p.n = 512;          % pixels
...
if ~p.fMRI && ~ismac
    ...
    wavwrite(corbeep, sr, 'cor_beep')
    [cor sr] = wavread('cor_beep');

    incorbeep = sin(2*pi*samp*3000); %1000 Hz
    wavwrite(incorbeep, sr, 'incor_beep')
    [incor sr] = wavread('incor_beep');
end
...
for blk = p.runNum
    ...
    if p.fMRI
        fName=[p.root, '\Subject Data\', p.subName, '_subNum', num2str(p.subNum), '_allAttn_fMRI_Session', num2str(p.sessNum), '_Block', num2str(blk), '.mat'];   %MS change filename
    else
        fName=[p.root, '\Subject Data\', p.subName, '_subNum', num2str(p.subNum), '_allAttn_Training_Session', num2str(p.sessNum), '_Block', num2str(blk), '.mat'];   %MS change filename
    end
