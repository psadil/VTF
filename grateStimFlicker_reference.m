function grateStimFlicker_reference
%208 TRs @ 1.5s/TR
...
p.fullScreen = 0;                     % if 1 then the display will be in a small window, use this for debugging.
...
for b=1:p.nBlocks
   ...
   fName=[p.root, '\Subject Data\', p.subName, '_train', num2str(p.scanNum), '.mat'];
   ...
   if exist(fName,'file')
    ...
    %space bar)
    while 1
       [resp, timeStamp] = checkForResp([27,32,84]);
       if resp==32 || resp==84
           break;
       end
    ...
    resp = 0;
    while resp~=67
        [resp, timeStamp] = checkForResp([27,67]);
        if resp==-1; ListenChar(0); return; end;
    end
    Screen('Flip', w);
end % end block loop
